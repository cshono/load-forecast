...
def test_imputer_handles_missing_values():
    ...
    pipeline = create_preprocessing_pipeline(numeric_cols=['feature1', 'feature2'], categorical_cols=['hour'], datetime_column='datetime')
    transformed_data = pipeline.fit_transform(data_with_nans)
    assert not np.any(np.isnan(transformed_data)), "Imputer did not handle missing values correctly."
...
def test_add_time_features():
    ...
    assert all(col in transformed_data.columns for col in expected_columns), "Time features not added correctly."
...
def test_scaling():
    ...
    pipeline = create_preprocessing_pipeline(numeric_cols=['feature1', 'feature2'], categorical_cols=['hour'], datetime_column='datetime')
    transformed_data = pipeline.fit_transform(data)
    assert np.allclose(np.min(transformed_data, axis=0), 0, atol=1e-7), "Data min is not scaled to 0 after scaling."
    assert np.allclose(np.max(transformed_data, axis=0), 1, atol=1e-7), "Data max is not scaled to 1 after scaling."
