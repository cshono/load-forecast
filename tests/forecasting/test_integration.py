...
def test_full_pipeline_workflow_w_grid_search():
    ...
    forecasting_api = ForecastingAPI(model_type='xgboost', preprocessing_pipeline=preprocessing_pipeline)
    ...
    assert forecasting_api.grid_search_result.best_estimator_ is not None, "model best_estimator has been set"
    assert isinstance(forecasting_api.grid_search_result, GridSearchCV), "forecast api returns grid_search_result"
